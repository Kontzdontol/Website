document.addEventListener("DOMContentLoaded", () => {
  ...
  imageGenerateBtn.addEventListener("click", async () => {
    ...
    reader.onload = async () => {
      ...
      try {
        const res = await fetch("https://ai-backend-production-ddef.up.railway.app/generate-image", {
          method: "POST",
          headers: {
  ...
  async function pollForResult(url) {
    for (let i = 0; i < 60; i++) {
      ...
      const poll = await fetch(`https://ai-backend-production-ddef.up.railway.app/poll?url=${encodeURIComponent(url)}`);
